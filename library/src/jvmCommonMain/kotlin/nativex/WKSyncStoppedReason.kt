...
import com.blockset.walletkit.nativex.library.WKNativeLibraryDirect.wkSyncStoppedReasonGetMessage
...
internal class WKSyncStoppedReason : Structure {
    @JvmField
    var typeEnum = 0
    @JvmField
    var u: u_union? = null
    ...
    fun type(): WKSyncStoppedReasonType {
    ...
    override fun read() {
        super.read()
        if (type() === WKSyncStoppedReasonType.POSIX) u!!.setType(u_union.posix_struct::class.java)
        u!!.read()
    }

    val message: Optional<String>
        get() {
            val ptr = wkSyncStoppedReasonGetMessage(this)
            return try {
